...
func Drush(args []string, flg flags.Accumulator) error {
	...
	rc, err := api.PostRemoteCommandDrush(creds, seCtx.Subscription.HashedID, seCtx.Environment.HashedID, strings.Join(args, " "), envVarKeyValue(flg.EnvironmentVars), timeout)
	if err != nil {
		return err
	}
